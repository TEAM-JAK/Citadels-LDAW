...
export function GetPlayerOrderPlayPhase (G, ctx) {
  ...
  let playerOrder = Object.keys(dict).sort(function(a,b){return dict[a] - dict[b]});
  if (ctx.numPlayers < 4) {
...
export function IsPlayPhaseOver(G, ctx) {
  ...
  let deckCount = G.deckOfCharacters.length + G.faceDownCharacterCards.length + G.faceUpCharacterCards.length;
  return deckCount === 8;
}
